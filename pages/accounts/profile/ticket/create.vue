            ...
            <div class="panel__box d-flex">
              ...
              <div class="col-12 col-md-9 info-content">
                <div>
                  ...
                  <select required name="orderId" id="orderId" v-model="orderId">
                    <option v-for="order in orders" v-bind:key="order" :value="order._id">{{ order.tracking_code }}</option>
                  </select>
                </div>
                <div>
                ...
                <button v-on:click="newTicket()" class="btn btn-success">ثبت</button>
...
export default {
  ...
  data() {
    return {
      ...
      title: '',
      text: '',
      attachmentsURL: ' ssdfsd dfsd fsdf ',
      emptyfield: false,
      orders: [],
    }
  },
  ...
  methods: {
    getOrders() {
      this.$store.dispatch('orders/getOrders').then((result) => {
        this.orders = result
        console.log(result)
      })
    },
